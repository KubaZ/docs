...
versions:
  feature: copilot
redirect_from:
  - /copilot/how-tos/use-copilot-agents/coding-agent/customize-the-agent-environment
...
## About customizing {% data variables.copilot.copilot_cloud_agent %}'s development environment
...
> [!NOTE]
> The `copilot-setup-steps` job does not support calling reusable workflows with `uses`. Define setup logic directly in the job's `steps` instead.
...
> [!NOTE]
> * {% data variables.copilot.copilot_cloud_agent %} is only compatible with Ubuntu x64 Linux and Windows 64-bit runners. Runners with macOS or other operating systems are not supported.
...
1. If you want to configure a proxy server for {% data variables.copilot.copilot_cloud_agent %}'s connections to the internet, configure the following environment variables as appropriate:
